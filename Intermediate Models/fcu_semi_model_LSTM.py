...
# DataLoader function
def create_dataloader(data, batch_size=128, sequence_length=4):
...
class LSTMAutoencoder(nn.Module):
    def __init__(self, input_dim, sequence_length=4):
...
def train_model_kfold(train_data, model_save_path, scaler, k=5):
    ...
    # Ask user if they want to use a semi-supervised approach
    semi_supervised = input("Do you want to use a semi-supervised approach? (yes/no): ").strip().lower()

    faulty_data = None
    if semi_supervised == "yes":
        faulty_filepath = input("Enter filepath of labeled faulty data: ").strip()
        if os.path.exists(faulty_filepath):
            faulty_df = load_data(faulty_filepath)
            faulty_data, _ = preprocess_data(faulty_df, scaler=scaler, fit_scaler=False)
            print("Labeled faulty data loaded successfully.")
        else:
            print("Warning: Faulty data file not found. Proceeding with unsupervised training.")
            faulty_data = None
    ...
    for train_idx, val_idx in kfold.split(train_data):
        ...
        train_loader = create_dataloader(train_subset, sequence_length=4)
        val_loader = create_dataloader(val_subset, sequence_length=4)

        # Initialize a fresh model for each fold
        model = LSTMAutoencoder(input_dim=input_dim, sequence_length=4).to(device)
        ...
        # Determine threshold using validation losses
        mean_loss = np.mean(val_losses)
        std_loss = np.std(val_losses)
        threshold = mean_loss + std_loss  # Default unsupervised threshold

        # If semi-supervised, refine threshold using faulty data
        if faulty_data is not None:
            print("Refining anomaly threshold using faulty data...")
            faulty_loader = create_dataloader(faulty_data, sequence_length=4)
            faulty_losses = []

            with torch.no_grad():
                for batch in faulty_loader:
                    batch = batch[0].to(device)
                    output = model(batch)
                    loss = criterion(output, batch).item()
                    faulty_losses.append(loss)

            # Set threshold at the minimum loss from faulty data to separate normal/faulty cases
            if len(faulty_losses) > 0:
                min_faulty_loss = np.min(faulty_losses)
                threshold = (mean_loss + std_loss + min_faulty_loss) / 2
                print(f"Adjusted threshold using faulty data: {threshold:.6f}")
    ...
    train_loader = create_dataloader(train_data, sequence_length=4)
    model = LSTMAutoencoder(input_dim=input_dim, sequence_length=4).to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.0001)
    criterion = nn.MSELoss()
...
def test_model(model_load_path, test_filepaths, predictions_output_path):
    ...
    model = LSTMAutoencoder(input_dim=16, sequence_length=4).to(device)
    ...
    for test_filepath in test_filepaths:
        ...
        test_loader = create_dataloader(test_data, batch_size=1, sequence_length=4)
